...
TEST_CASE("M,N,K <= 8", "[.benchmark][small]") {
	...
	bench.run("naive", [=] { 
		util::naive_gemm(M, N, K, alpha, ptr_A, K, ptr_B, N, beta, ptr_C, N);
	});
	std::copy(oldC.begin(), oldC.end(), C.begin());

	bench.run("blas", [=] {
		cblas_sgemm(CBLAS_ORDER::CblasRowMajor, CBLAS_TRANSPOSE::CblasNoTrans, CBLAS_TRANSPOSE::CblasNoTrans, M, N, K, alpha, ptr_A, K, ptr_B, N, beta, ptr_C, N);
	});
}
...
TEST_CASE(">= 1024", "[.benchmark][very large]") {
	...
	const std::string title = fmt::format("{0}x{0}", dim);
	fmt::print(*util::benchmark_output, "\n### {}\n", title);

	bench.title(title);
	bench.run("gemm", [=] { 
		gemm::sgemm(gemm::transposition::none, gemm::transposition::none, dim, dim, dim, alpha, ptr_A, dim, ptr_B, dim, beta, ptr_C, dim);
	});
	std::copy(oldC.begin(), oldC.end(), C.begin());

	bench.run("blas", [=] {
		cblas_sgemm(CBLAS_ORDER::CblasRowMajor,
	      CBLAS_TRANSPOSE::CblasNoTrans, CBLAS_TRANSPOSE::CblasNoTrans, dim,
	      dim, dim, alpha, ptr_A, dim, ptr_B, dim, beta, ptr_C, dim);
	});
}
